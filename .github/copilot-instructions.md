# draft_helper Development Guidelines

Auto-generated from all feature plans. Last updated: 2025-10-12

## Active Technologies
- Vite + vanilla JavaScript (ES2022, compiled to ES2020 for browser compatibility) + Alpine.js (~15kb reactivity), Tailwind CSS + DaisyUI (styling), Tabulator (data tables) (002-foundation)
- TypeScript + ES2022 (compiled to ES2020 for browser compatibility) + Alpine.js (reactivity), Tailwind CSS + DaisyUI (styling), Tabulator (data tables) (003-data-integration)
- LocalForage (enhanced localStorage) - client-side only (003-data-integration)

## Project Structure
```
backend/
frontend/
tests/
```

## Commands
npm test [ONLY COMMANDS FOR ACTIVE TECHNOLOGIES][ONLY COMMANDS FOR ACTIVE TECHNOLOGIES] npm run lint

## Code Style
Vite + vanilla JavaScript (ES2022, compiled to ES2020 for browser compatibility): Follow standard conventions

## Recent Changes
- 003-data-integration: Added TypeScript + ES2022 (compiled to ES2020 for browser compatibility) + Alpine.js (reactivity), Tailwind CSS + DaisyUI (styling), Tabulator (data tables)
- 002-foundation: Added Vite + vanilla JavaScript (ES2022, compiled to ES2020 for browser compatibility) + Alpine.js (~15kb reactivity), Tailwind CSS + DaisyUI (styling), Tabulator (data tables)

<!-- MANUAL ADDITIONS START -->
<!-- MANUAL ADDITIONS END -->
