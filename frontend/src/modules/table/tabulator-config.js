export function createTableConfig(data) {
  ...
  { title: 'FG%', field: 'fg_pct', sorter: 'number', width: 80, formatter: (cell) => (cell.getValue() == null ? '' : (cell.getValue()*100).toFixed(1) + '%') },
  { title: 'FT%', field: 'ft_pct', sorter: 'number', width: 80, formatter: (cell) => (cell.getValue() == null ? '' : (cell.getValue()*100).toFixed(1) + '%') },
